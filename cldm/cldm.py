...
class ControlledUnetModel(UNetModel): # Base SD
...
class ControlNet(nn.Module): # clone SD 
    def __init__(
            self,
            image_size,
            in_channels,
...
class ControlLDM_IPA(LatentDiffusion):
    ...
    @torch.no_grad()
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        x, ct, ci = super().get_input(batch, self.first_stage_key, *args, **kwargs)
        control = batch[self.control_key]
        if bs is not None:
            control = control[:bs]
        control = control.to(self.device)
        control = einops.rearrange(control, 'b h w c -> b c h w')
        control = control.to(memory_format=torch.contiguous_format).float()
        return x, dict(c_crossattn=[ct, ci], c_concat=[control]) 

    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        assert isinstance(cond, dict)
        diffusion_model = self.model.diffusion_model

        cond_txt = torch.cat(cond['c_crossattn'][0], 1)
